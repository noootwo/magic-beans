// 动态按需加载 sharp，避免在浏览器或未使用相关功能时引入重量级依赖
let SharpCtor: any | null = null;
async function getSharpCtor() {
  if (!SharpCtor) {
    const mod = await import("sharp");
    SharpCtor = (mod as any).default;
  }
  return SharpCtor as (input?: any, options?: any) => any;
}
...
export async function loadImageFromSource(
  ...
  try {
    const Sharp = await getSharpCtor();
